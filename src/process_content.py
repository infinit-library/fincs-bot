...
from .storage import connect_db, insert_parsed_event, insert_raw_snapshot, sha256_text, utcnow
...
def classify_and_parse(seg: str) -> dict:
    ...
    is_trading = bool(side or pair or entry_price or tp_price or sl_price)
    action = "ENTRY" if is_trading else None
    is_add = "ADD" in norm or "追加" in s  # lightweight add-on marker
...
def save_snapshot_and_segments(raw_text: str, channel: str = "NOBU_CHANNEL"):
    ...
    conn = connect_db("data/fincs.db")

    # 1) Save raw snapshot (dedupe by raw_hash)
    ...
    # 2) Save segments (dedupe by segment_hash)
